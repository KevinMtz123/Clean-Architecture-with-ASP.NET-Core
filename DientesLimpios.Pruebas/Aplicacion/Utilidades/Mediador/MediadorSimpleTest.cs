...
namespace DientesLimpios.Pruebas.Aplicacion.Utilidades.Mediador
{
    ...
    public class MediadorSimpleTest
    {
        ...
        public class ValidatorRequestFalso : AbstractValidator<RequestFalso>
        {
            public ValidatorRequestFalso()
            {
        ...
        public async Task Send_ComandoNoValido_LanzaExcepcion()
        {
            ...
            var serviceProvider = Substitute.For<IServiceProvider>();
            var validador = new ValidatorRequestFalso();

            serviceProvider
                .GetService(typeof(IValidator<RequestFalso>)).Returns(validador);
